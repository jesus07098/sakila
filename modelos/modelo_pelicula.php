...
function obtenerInfoPeliculas($conexion)
{
    $sql = "select fi.film_id, fi.title , fi.description , fi.release_year , lan.name as idioma_oficial, la.name 
       as idioma_sec, fi.rental_duration, fi.rental_rate,fi.length, fi.replacement_cost, fi.rating, fi.special_features  
       from film as fi 
...
function insertarPelicula($conexion, $datos)
{
    ...
    VALUES (:titulo, :descripcion, :anoLanzamiento, :idioma, :idioma2, :rentaDuracion, :tasaArrendamiento, :tamano, 
            :costoReemplazo, :clasificacion, :caracteristicasEspeciales)';
